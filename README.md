...
> only. Instead, run these migration files, in order, the same way you'd
> import `schema.sql` (phpMyAdmin → your database → Import → choose the
> file → Go). Each is safe to run more than once and never deletes data:
> - `migrations/v1_to_v2.sql` — if you're coming from the very first
>   single-project-per-client version: moves each client's embedded
>   service/amount/status into the new `projects` table.
> - `migrations/v2_to_v3.sql` — adds the fields the exact-match invoice
>   design and manual invoice logging (below) need: a client contact
...
- **Clients** — add a client (name, contact, optional contact-person
  name/title for the "Attn:" line on invoices, GSTIN, state, address). A
...
- **Invoices** — generate a Proforma or Tax Invoice for any client,
  ...
  matches NikhilWorks' actual invoice format: CGST+SGST or IGST is
  calculated automatically depending on the client's state vs. your
  business state (on both Proforma and Tax invoices — a Proforma still
  shows the GST-inclusive total the client will actually pay), an
  optional advance/balance split ("Advance Payable Now (50%)" +
  "Balance") can be set per invoice, and a Proforma's "Valid Till" date
  is calculated from your validity-days setting. Print or "Save as PDF"
  from the browser's print dialog.
- **Log an already-issued invoice** — in the invoice form, tick "This
  invoice was already issued outside the app" to record one you'd
  already generated by hand (with its own number, like `NW/2026-27/001`)
  without touching the auto-numbering counters, so it shows up in your
  history and stays linked to the right client/project.
- **Letterhead** — write a one-off letter on NikhilWorks letterhead and
  print/save it as a PDF.
- **Settings** — your business details (including tagline and the name
  that signs invoices), bank details, Proforma validity period, and the
  invoice numbering prefixes/counters, editable any time.
